...
// FIXME - make it faster and current (e.g., rewrite it)
/**
...
public final class CdbMake {
    ...
    public static void make(
            String dataFilepath, String cdbFilepath,
     ...
     */
    public static void make(
            InputStream in, String cdbFilepath,
            String tempFilepath) throws IOException {
    ...
    private static int readInt(InputStream in, char terminator) throws IOException {
        int rv = 0;
    	int ch;
		while ((ch = in.read()) >= 0 && Character.isDigit(ch)) {
		    rv = (rv * 10) + Character.digit(ch, 10);
		}
		if (ch != terminator) {
			throw new IllegalArgumentException(
                    "input file not in correct format: " +
